...
4 philosophies, $0 start with origin stories, 10 tiers (100 levels),
venue time/perks/restrictions, match time allocation, overrun penalties,
day-of-week modifiers, intergender matches, tag title support,
tutorial system, iPhone UI with consolidated apps, seasonal events
"""
...
def get_active_seasonal_events(month, day):
    ...
    if month == 4 and 8 <= day <= 14:
        events.append({
            ...
            "xp_multiplier": 2.0, "attendance_multiplier": 1.5,
            "fan_growth_multiplier": 2.0, "color": "#f59e0b", "icon": "🏟️",
        })
    if month == 8 and 24 <= day <= 31:
        events.append({
            "name": "☀️ SummerSlam Week",
            "description": "Summer wrestling fever! Bonus fan growth!",
            "xp_multiplier": 1.5, "attendance_multiplier": 1.3,
            "fan_growth_multiplier": 1.5, "color": "#ef4444", "icon": "☀️",
        })
    if month == 1 and 15 <= day <= 21:
        events.append({
            "name": "👑 Rumble Season",
            "description": "Royal Rumble hype! Extra attendance boost!",
            "xp_multiplier": 1.3, "attendance_multiplier": 1.4,
            "fan_growth_multiplier": 1.3, "color": "#6366f1", "icon": "👑",
        })
    return events
...
def dashboard():
    ...
    for m_item in MONTHS:
        if isinstance(m_item, dict):
            month_names.append(m_item.get('name', f'Month {len(month_names)+1}'))
        else:
            month_names.append(str(m_item))
...
def start_tutorial():
    ...
    save_game_state(game_state)
    flash('📖 Tutorial started! Follow the steps.', 'success')
    return redirect(url_for('dashboard'))
...
def calendar_view():
    ...
    for y in range(1, view_year):
        for m_i in range(1, 13):
            total_days_before += days_in_month(m_i)
    for m_i in range(1, view_month):
        total_days_before += days_in_month(m_i)
