...
def dashboard():
    ...
    try:
        ...
        injured_count = len(injured_wrestlers)
    except Exception:
        injured_count = 0
        injured_wrestlers = []
    ...
    try:
        has_training_school = (
            game_state.training_school is not None
            and game_state.training_school.is_founded()
        )
    except Exception:
        has_training_school = False
    ...
    seasonal_events = current_events

    recent_show = getattr(game_state, 'last_show_result', None)

    calendar_widget_days = []
    try:
        today_day = getattr(promotion, 'current_day', 1)
        today_month = getattr(promotion, 'current_month', 1)
        today_year = getattr(promotion, 'current_year', 1)
        ...
        for offset in range(0, 7):
            ...
            calendar_widget_days.append({
                ...
                "month": month,
                "day": day,
                "label": format_date(year, month, day),
                "is_today": offset == 0,
                "day_name": get_day_name(get_day_of_week(year, month, day)),
            })
    except Exception:
        calendar_widget_days = []
    ...
    return render_template(
        ...
        injured_wrestlers=injured_wrestlers,
...
def group_detail(group_id):
    ...
    roster_lookup = {w.name: w for w in game_state.promotion.roster}
...
def free_agents():
    ...
    if player_level <= 5:
        ...
        allowed_tiers = [FreeAgentTier.ROOKIE, FreeAgentTier.PROSPECT, FreeAgentTier.RISING, FreeAgentTier.PROVEN]
    elif player_level <= 75:
        allowed_tiers = [FreeAgentTier.ROOKIE, FreeAgentTier.PROSPECT, FreeAgentTier.RISING, FreeAgentTier.PROVEN, FreeAgentTier.ELITE]
    else:
        allowed_tiers = [FreeAgentTier.ROOKIE, FreeAgentTier.PROSPECT, FreeAgentTier.RISING, FreeAgentTier.PROVEN, FreeAgentTier.ELITE, FreeAgentTier.INDY_GOD]
    ...
    for listing in all_listings:
        try:
            ...
            if isinstance(listing, dict):
                cost = listing.get('signing_bonus', 0) if listing.get('is_exclusive_offer', False) else listing.get('asking_per_show', 100)
            else:
                cost = listing.signing_bonus if getattr(listing, 'is_exclusive_offer', False) else listing.asking_per_show
...
def book_show():
    ...
    all_venues = get_all_venues() if limits.get("can_tour_international", False) else get_venues_by_continent(continent)
    ...
    eligible_venues.sort(key=lambda v: getattr(v, 'capacity', 0))

    show_all_venues = request.args.get('show_all', '0') == '1'
    hidden_venue_count = len([v for v in all_venues if getattr(v.tier, 'value', 1) > max_tier])
    venues = all_venues if show_all_venues else eligible_venues
    ...
    current_venue = get_venue_by_id(current_venue_id) if current_venue_id else None
    ...
    venue_day_mod = DEFAULT_DAY_MODIFIERS.get(show_day_name, {"label": "", "modifier": 1.0})
    ...
    if current_venue:
        venue_available_time = getattr(current_venue, 'time_limit_minutes', 120)
        estimated_venue_cost = getattr(current_venue, 'cost', getattr(current_venue, 'rental_cost', 0))
    else:
        venue_available_time = 120
        estimated_venue_cost = 0

    time_remaining = venue_available_time - card_total_time
    is_overrunning = card_total_time > venue_available_time
    overrun_minutes = max(0, card_total_time - venue_available_time)
    ...
    estimated_total_cost = estimated_venue_cost + estimated_salary_cost + estimated_production_cost + overrun_penalty
    ...
    return render_template(
        ...
        total_show_time=card_total_time,
        show_date=show_date,
        ...
        venue_time_limit=venue_available_time,
        available_minutes=venue_available_time,
        time_remaining=time_remaining,
        is_overrunning=is_overrunning,
        overrun_minutes=overrun_minutes,
        minutes_over=overrun_minutes,
        overrun_penalty=overrun_penalty,
        overrun_fine=overrun_penalty,
...
def select_venue(venue_id):
    ...
    if getattr(venue.tier, 'value', 1) > max_tier:
        flash('This venue is locked at your current level.', 'error')
        return redirect(url_for('book_show'))
...
@app.route('/change-venue', methods=['POST'])
@require_login
@require_game
def change_venue():
    session['current_venue_id'] = None
    flash('Venue cleared. Choose another venue.', 'info')
    return redirect(url_for('book_show'))
...
def add_match():
    ...
    if len(current_card) >= max_matches:
        flash(f'Match card is full. Max matches: {max_matches}', 'error')
        return redirect(url_for('book_show'))
    ...
    if len(participants) < match_info.get('min', 2):
        flash(f'{match_type} needs at least {match_info.get("min", 2)} wrestlers.', 'error')
        return redirect(url_for('book_show'))
    ...
    clashes = [name for name in participants if name in already_booked]
    if clashes:
        flash(f'Already booked tonight: {", ".join(clashes)}', 'error')
        return redirect(url_for('book_show'))
...
@app.route('/remove-match/<int:index>', methods=['GET', 'POST'])
@app.route('/remove-match/<int:match_index>', methods=['GET', 'POST'])
@require_login
@require_game
def remove_match(index=None, match_index=None):
    idx = index if index is not None else match_index
    current_card = session.get('current_card', [])

    if idx is not None and 0 <= idx < len(current_card):
        removed = current_card.pop(idx)
...
def reorder_matches():
    ...
    try:
        ...
        to_slot = int(request.form.get('to_slot', request.form.get('to_index', -1)))
    except Exception:
...
def run_show():
    ...
    if not venue_id or not card:
        flash('No booked show found.', 'error')
        return redirect(url_for('book_show'))
...
def award_title(championship_id):
    ...
    championship = champ_manager.get_championship(championship_id) if champ_manager else None
    ...
    if request.method == 'POST':
        ...
        save_game_state(game_state)
        flash(f'{championship.name} awarded.', 'success')
        return redirect(url_for('championships'))

    return render_template(
        'award_title.html',
        championship=championship,
        wrestlers=promotion.roster,
...
def calls_app():
    ...
    if not game_state.calls:
        game_state.calls = CallsManager()

    if not game_state.banking:
        game_state.banking = BankingManager()
...
def take_loan(loan_type=None, loan_id=None):
    ...
    try:
        loan_type_enum = LoanType.LOAN_SHARK if str(loan_type_str).lower() in ['loan_shark', 'loanshark', 'shark'] else LoanType.BANK
...
def injury_report():
    ...
    if not game_state.injury_manager:
        game_state.injury_manager = InjuryManager()
        save_game_state(game_state)
...
def ensure_training_school_systems(game_state):
    if not getattr(game_state, 'training_school', None):
        game_state.training_school = TrainingSchool()

    if not getattr(game_state, 'coach_manager', None):
        game_state.coach_manager = CoachManager()

    if not getattr(game_state, 'coach_pool', None):
        game_state.coach_pool = CoachPool()

    if not getattr(game_state, 'trainee_pool', None):
        game_state.trainee_pool = TraineePool()

    if not getattr(game_state, 'trainee_show_manager', None):
        game_state.trainee_show_manager = TraineeShowManager()
...
def found_school():
    ...
    if request.method == 'POST':
        name = request.form.get('name', '').strip() or f"{promotion.name} Dojo"
...
def trainee_show():
    ...
    return render_template(
        ...
        active_trainee_count=len(getattr(school, 'trainees', [])),
        budget=game_state.promotion.budget,
        currency=getattr(game_state, 'game_settings', {}).get("currency_symbol", "$"),
        hide_base_hud=True,
    )
