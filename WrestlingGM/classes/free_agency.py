...
class AgentListing:
    ...
    asking_per_show: int = 500       # Per-appearance fee
    asking_weekly_salary: int = 0    # Only for exclusive contracts
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "hometown": w.hometown,
            "level": w.wrestler_level.value,
            "level_icon": w.get_level_icon(),
            "level_color": w.get_level_color(),
            "alignment": w.alignment.value,
            "alignment_icon": w.get_alignment_icon(),
            "primary_style": w.primary_style.value,
            "secondary_style": w.secondary_style.value if w.secondary_style else None,
    ...
    def from_dict(cls, data: dict) -> "AgentListing":
        ...
        return cls(
            ...
            contract_type=ct,
            asking_per_show=data.get("asking_per_show", 500),
...
class FreeAgencyManager:
    ...
    def _create_listing_for_wrestler(
        ...
        is_indy_god = wrestler.is_indy_god
        is_licensed = wrestler.has_trait("Licensed Talent")
    ...
    def _calculate_contract_terms(self, wrestler: Wrestler) -> Tuple[int, int, int, bool]:
        ...
        # Use wrestler's calculated booking fee as base
        base_fee = wrestler.calculate_booking_fee()
        ...
        if wrestler.is_indy_god and random.random() < 0.5:
            is_exclusive = False
        ...
        if is_exclusive:
            # Calculate bonus and contract length scaled by level
            level_mult = LEVEL_INFO.get(wrestler.wrestler_level, {}).get("fee_multiplier", 1.0)
            signing_bonus = int(base_fee * level_mult * 4)  # ~4x weekly fee as bonus
    ...
    def sign_wrestler(
        ...
        # Apply contract terms to wrestler
    ...
    def _process_rival_interest(
        ...
        try:
            active_rivals = rival_promotions.get_all_rivals() if hasattr(rival_promotions, 'get_all_rivals') else []
        except Exception:
            active_rivals = []
